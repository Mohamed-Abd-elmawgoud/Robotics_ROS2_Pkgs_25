...
class ForwardKinematicsNode(Node):
    def __init__(self):
        ...
        # Fixed offset (EE_frame relative to Gripper) in meters
        self.ee_offset = np.array([0.007, 0.1, 0.013]) * 1000.0  # convert to mm for FK consistency
        ...
        self.get_logger().info("Forward Kinematics Node started (with EE_frame offset).")
    ...
    # ------------------------------------------------------------
    # Apply FK and publish EE_frame pose
    # ------------------------------------------------------------
    def compute_and_publish_fk(self):
        ...
        # Compute FK to gripper
        T = self.fwd_kinematics(self.q)

        # Apply EE_frame offset (in mm)
        ee_offset_h = np.array([*self.ee_offset, 1.0])
        ee_in_base = T @ ee_offset_h  # transform gripper→EE_frame

        # Convert to meters for ROS
        pos = ee_in_base[:3] / 1000.0
